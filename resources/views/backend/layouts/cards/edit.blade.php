    ...
    <div class="content">
        ...
        <form action="{{ route('cards.update', $card->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <img src="{{ asset('storage/' . $card->image) }}" alt="Card Image" width="100" height="100">
            </div>
            ...
            <div class="mb-3">
                <label for="seller" class="form-label">Seller</label>
                <input type="text" class="form-control" id="seller" name="seller" value="{{ $card->seller }}">
                @error('seller') <p class="text-danger">{{ $message }}</p> @enderror
            </div>
            ...
            <div class="mb-3">
                <label for="base_price" class="form-label">Base Price</label>
                <input type="number" class="form-control" id="base_price" name="base_price" value="{{ $card->base_price }}" step="0.01">
                @error('base_price') <p class="text-danger">{{ $message }}</p> @enderror
            </div>

            <!-- Discount -->
            <div class="mb-3">
                <label for="discount" class="form-label">Discount</label>
                <input type="number" class="form-control" id="discount" name="discount" value="{{ $card->discount }}" step="0.01">
            ...
            <div class="mb-3">
                ...
                <select class="js-example-basic-multiple form-control" name="versions[]" multiple="multiple">
                    @foreach ($versions as $version)
                        <option value="{{ $version->id }}" {{ $card->versions->contains($version->id) ? 'selected' : '' }}>
                            {{ $version->name }}
                        </option>
                    @endforeach
                </select>
                @error('versions') <p class="text-danger">{{ $message }}</p> @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="js-example-basic-multiple form-control" name="amounts[]" multiple="multiple">
                    @foreach ($amounts as $amount)
                        <option value="{{ $amount->id }}" {{ $card->amounts->contains($amount->id) ? 'selected' : '' }}>
                            ${{ $amount->value }}
                        </option>
                    @endforeach
                </select>
                @error('amounts') <p class="text-danger">{{ $message }}</p> @enderror
            </div>
            ...
            <div class="mb-3">
                <label for="usage" class="form-label">Usage</label>
                <input type="text" class="form-control" id="usage" name="usage" value="{{ $card->usage }}" required>
                @error('usage') <p class="text-danger">{{ $message }}</p> @enderror
            </div>

            <!-- Description -->
            <div class="mb-3">
                <label for="description" class="form-label">Description</label>
                <input type="text" class="form-control" id="description" name="description" value="{{ $card->description }}" required>
            ...
            <div class="mb-3">
                <label for="delivery_time" class="form-label">Delivery time</label>
                <input type="text" class="form-control" id="delivery_time" name="delivery_time" value="{{ $card->delivery_time }}">
                @error('delivery_time') <p class="text-danger">{{ $message }}</p> @enderror
            </div>
